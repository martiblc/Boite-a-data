...
-   Sauvegarder résultat fonction dans environnement global R (à utiliser dans fonction par exemple)
...
-   [Filtrer dans plusieurs variables](https://dplyr.tidyverse.org/reference/filter_all.html#arguments)
    -   if_any() si la condition doit être remplie dans une seule des variables au moins
    -   if_all() si elle doit l'être dans toutes les variables

Les deux se basent sur une fonction pour filter. Il faut donc l'introduire avec le tilde. Le .x avant un opérateur est nécessaire pour lui introduire chacune des variables que va itérer le everything() ou across().
...
Utiliser str_detect avec filter - str_detect(df,"\^F")) -\> commence par F - "F\$" -\> finit par F - "\\d" -\> contient des chiffres - ! -\> placé devant str_detect comme argument "SAUF"
